...
def train_squad(args, tokenizer, model):
    ...
    if not args.load_data_from_pt:
        train_dataset, train_dataloader = load_squad_to_torch_dataset(path_input_data=args.path_input_train_data,
                                                                      tokenizer=tokenizer,
    ...
    optimier_grouped_parameters = [
        {'params': [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay)],
         'weight_dacay': args.weight_decay},
        {'params': [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}
    ]
    optimizer = AdamW(optimier_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
            ...
            if (step+1) % args.gradient_accumulation_steps == 0 or (step == len(train_dataloader) - 1):
                ...
                if (args.save_steps > 0 and global_step % args.save_steps == 0 or \
                    ...
                    torch.save({
                        ...
                        'loss': output_train['loss'],
                        'acc': output_train['accuracy'],
                        'f1': output_train['f1']
                    }, os.path.join(output_dir, 'torch_model_checkpoint.pt'))
                    tokenizer.save_pretrained(output_dir)
